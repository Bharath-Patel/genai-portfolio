...
def ask(question: str, max_retries: int = 3):
    ...
    for attempt in range(max_retries):
        try:
            retry_temp = 0.0 if attempt == 0 else 0.3
            llm_for_attempt = ChatGroq(
                model="llama-3.1-8b-instant",
                temperature=retry_temp,
                api_key=os.getenv("GROQ_API_KEY")
            )
            agent_for_attempt = create_agent(
                llm_for_attempt,
                tools=[knowledge_base_search],
                system_prompt="You are a helpful assistant with access to a technical knowledge base. Call the knowledge base at most once per new question."
            )
            result = agent_for_attempt.invoke(
